...
def make_anchor_generator_atss(cfg):
    assert len(cfg.anchor_strides) == len(cfg.anchor_sizes), "Only support FPN now"
    new_anchor_sizes = []

    for size in cfg.anchor_sizes:
        per_layer_anchor_sizes = []
        for aa in range(cfg.scales_per_octave):
            octave_scale = cfg.octave ** (aa / float(cfg.scales_per_octave))
            per_layer_anchor_sizes.append(octave_scale * size)

        new_anchor_sizes.append(tuple(per_layer_anchor_sizes))

    anchor_generator = AnchorGenerator(
        tuple(new_anchor_sizes), cfg.aspect_ratios, cfg.anchor_strides, cfg.straddle_thre
    )
