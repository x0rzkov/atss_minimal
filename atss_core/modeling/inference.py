...
class ATSSPostProcessor(torch.nn.Module):
    def __init__(self, cfg, box_coder):
        super().__init__()
        self.pre_nms_thresh = cfg.test_conf_thre
        self.pre_nms_top_n = cfg.pre_nms_top_n
        self.nms_thresh = cfg.nms_thre
        self.post_nms_top_n = cfg.post_nms_top_n
        self.min_size = 0
        self.num_classes = cfg.num_classes
        self.bbox_aug_enabled = False
        self.box_coder = box_coder
        self.bbox_aug_vote = False
    ...
    def forward(self, box_cls, box_regression, centerness, anchors):
        ...
        for _, (o, b, c, a) in enumerate(zip(box_cls, box_regression, centerness, anchors)):
            sampled_boxes.append(self.forward_for_single_feature_map(o, b, c, a))
